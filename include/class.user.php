...
class UserModel extends VerySimpleModel {
    ...
    static protected $perms = array(
        self::PERM_CREATE => array(
            'title' => /* @trans */ 'Crear',
            'desc' => /* @trans */ 'Capacidad para agregar nuevos usuarios',
            'primary' => true,
        ),
        self::PERM_EDIT => array(
            'title' => /* @trans */ 'Editar',
            'desc' => /* @trans */ 'Capacidad para gestionar la información del usuario',
            'primary' => true,
        ),
        self::PERM_DELETE => array(
            'title' => /* @trans */ 'Borrar',
            'desc' => /* @trans */ 'Posibilidad de eliminar usuarios',
            'primary' => true,
        ),
        self::PERM_MANAGE => array(
            'title' => /* @trans */ 'Administrar cuenta',
            'desc' => /* @trans */ 'Capacidad para administrar cuentas de usuarios activos',
            'primary' => true,
        ),
        self::PERM_DIRECTORY => array(
            'title' => /* @trans */ 'Directorio de usuarios',
            'desc' => /* @trans */ 'Posibilidad de acceder al directorio de usuarios',
            'primary' => true,
        ),
    );
...
implements TemplateVariable, Searchable {
    ...
    static function fromForm($form, $create=true) {
        ...
        if (($field=$form->getField('email'))
            ...
            $field->addError(__('El correo electrónico está asignado a otro usuario'));
            $valid = false;
        }
        ...
        try {
            ...
            foreach ($records as $data) {
                if (!Validator::is_email($data['email']) || empty($data['name']))
                    throw new ImportError('Ambos `name` y `email` los campos son obligatorios');
                if (!($user = static::fromVars($data, true, true)))
                    throw new ImportError(sprintf(__('No se puede importar el usuario: %s'),
    ...
    function updateInfo($vars, &$errors, $staff=false) {
        ...
        foreach ($forms as $entry) {
            ...
            elseif ($entry->getDynamicForm()->get('type') == 'U'
                    ...
                    && $u->id != $this->getId()) {
                $valid = false;
                $f->addError(__('El correo electrónico está asignado a otro usuario'));
            }
...
class UserAccount extends VerySimpleModel {
    ...
    protected function sendUnlockEmail($template) {
        ...
        if (!$email ||  !$content)
            return new BaseError(sprintf(_S('%s: No se puede recuperar la plantilla'),
                $template));
    ...
    function update($vars, &$errors) {
        ...
        if ($vars['passwd1'] || $vars['passwd2']) {
            if (!$vars['passwd1'])
                $errors['passwd1'] = __('Se requiere una nueva contraseña');
            else {
                try {
        ...
        if ($vars['username'] && Validator::is_email($vars['username']))
            $errors['username'] =
                __('Los usuarios siempre pueden iniciar sesión con su dirección de correo electrónico');
    ...
    static function register($user, $vars, &$errors) {
        ...
        if ((!$vars['backend'] || $vars['backend'] != 'client')
            ...
            if (!$vars['passwd1'])
                $errors['passwd1'] = 'Se requiere contraseña temporal';
...
class UserAccountStatus {
    ...
    function __toString() {
        ...
        if ($this->isLocked())
            return __('Bloqueado (administrativo)');

        if (!$this->isConfirmed())
            return __('Bloqueado (activación pendiente)');

        // ... Other flags here (password reset, etc).

        return __('Activo (Registrado)');
    }
}
