<div class="row">
<div class="span8">
    <h1><?php echo __('Preguntas frecuentes');?></h1>
    <div><strong><?php echo __('Resultados de la búsqueda'); ?></strong></div>
<?php
    if ($faqs->exists(true)) {
        echo '<div id="faq">'.sprintf(__('%d preguntas frecuentes coincidieron con tus criterios de búsqueda.'),
            $faqs->count())
            .'<ol>';
        foreach ($faqs as $F) {
            echo sprintf(
        ...
        echo '<strong class="faded">'.__("La búsqueda no coincide con ninguna pregunta frecuente".).'</strong>';
    }
?>
</div>
...
<?php } ?>
        ...
        <section>
            <div class="header"><?php echo __('Categorías'); ?></div>
<?php
