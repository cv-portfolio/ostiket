...
implements ArrayAccess, Serializable {
    ...
    function __construct($array=array()) {
        if (!is_array($array) && !$array instanceof Traversable)
            throw new InvalidArgumentException('Se espera un objeto o matriz transitable');
        foreach ($array as $v)
            $this->storage[] = $v;
    }
    ...
    function remove($value) {
        if (!($k = $this->index($value)))
            throw new OutOfRangeException('No existe tal elemento en la lista');
    ...
    function pop($at=false) {
        ...
        elseif (!isset($this->storage[$at]))
            throw new OutOfRangeException('Índice fuera de rango');
        else {
    ...
    function offsetGet($offset) {
        if (!is_int($offset))
            throw new InvalidArgumentException('Los índices de la lista deben ser números enteros');
        elseif ($offset < 0)
            $offset += count($this->storage);
        if (!isset($this->storage[$offset]))
            throw new OutOfBoundsException('Índice de lista fuera de rango');
    ...
    function offsetSet($offset, $value) {
        ...
        if (!isset($this->storage[$offset]))
            throw new OutOfBoundsException('Asignación de lista fuera de rango');
...
implements TemplateVariable {
    ...
    function add($recipient) {
        if (!$recipient instanceof EmailRecipient)
            throw new InvalidArgumentException('Se espera el destinatario del correo electrónico');
    ...
    static function getVarScope() {
        return array(
            'names' => __('Lista de nombres'),
            'emails' => __('Lista de direcciones de correo electrónico'),
            'full' => __('Lista de nombres y direcciones de correo electrónico'),
        );
    }
}
