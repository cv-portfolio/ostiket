...
class Translation extends gettext_reader implements Serializable {
    ...
    function __construct($reader, $charset=false) {
        ...
        // Just load the cache
        if (!is_string($reader))
            throw new RuntimeException('Error de programación: Nombre de archivo esperado para la fuente de traducción');
        $this->STREAM = $reader;
    ...
    static function buildHashFile($mofile, $outfile=false, $return=false) {
        ...
        if (!$stream)
            throw new InvalidArgumentException(
              'Se esperaba un nombre de archivo o recurso válido');
        ...
        if ($reader->short_circuit || $reader->error)
            throw new Exception('No se puede inicializar el archivo de entrada MO');

        $reader->load_tables();

        // Get basic table
        if (!($table = $reader->cache_translations))
            throw new Exception('No se pueden leer las traducciones del archivo');
...
class CustomDataTranslation extends VerySimpleModel {
    ...
    function decodeComplex($text) {
        ...
        case 'A':
            ...
            break;
        default:
            throw new Exception($version . ': Formato complejo desconocido');
        }
    ...
    function getComplex() {
        if (!$this->flags && self::FLAG_COMPLEX)
            throw new Exception('Error de coherencia de datos. La traducción no es compleja');
